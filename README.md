...
- 音频：9 段情绪背景音乐（Kevin MacLeod，CC-BY 4.0，见 `frontend/public/audio/CREDITS.md`）+ Web Audio 实时合成交互音效。
- 朗读：优先腾讯云情感语音合成 TTS（可选），未配置或失败时降级浏览器原生 `speechSynthesis`。
...
| `TENCENT_TTS_REGION` / `TENCENT_TTS_VOICE_TYPE` / `TENCENT_TTS_TIMEOUT` | 可选：TTS 区域、音色与超时（默认 `ap-guangzhou` / `101016` 温柔女声 / 8s）。 |
...
8. （可选）打开记忆地图 / 时光机回望，展示岛屿生长轨迹；或写一个心境字让岛屿读心。
9. 输入高风险文本：`我真的彻底绝望崩溃了，完全撑不下去了，一点希望都没有，太无助了`，展示安全提示：暂停普通叙事，引导联系可信任的人或专业热线。
10. 展示隐私可控：在身份入口一键删除该身份的后端记忆，岛屿清空。
...
在 2D 之上，可叠加 **react-three-fiber 真 3D 岛屿皮肤**（`src/components/Island3D.tsx`）与可漫游的**自由探索模式**（`src/components/ExploreMode.tsx`，GLB 模型在 `frontend/public/models/`）。3D 按设备性能与 WebGL 支持自动分级，弱设备或不支持时回退 2D。
...
**交互音效**由 `src/lib/sfx.ts` 通过 Web Audio API 实时合成，无外部素材、无版权依赖。
...
可选开启腾讯云情感语音合成（让「朗读叙事」升级为云端情感音色）：
