...
完整需求与设计见 [`心屿-需求文档.md`](心屿-需求文档.md)（v5.0，完整自包含版，依据仓库实际实现回写）。
...
AI 已配置真实模型 Key，默认走**真实 LLM**（情绪分析 / 叙事表达）；同时保留完整降级兜底——未配置 Key 或调用失败时自动回落 Mock，情绪、叙事、印记、岛屿、安全均能完整跑通，演示不中断。支持情绪：`sad`、`anxious`、`tired`、`lonely`、`calm`、`happy`、`angry`、`helpless`。
