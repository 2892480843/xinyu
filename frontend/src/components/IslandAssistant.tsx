...
export default function IslandAssistant({ userId }: { userId: string }) {
    ...
    <>
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            <motion.div
              className="panel-glass-2 rounded-card p-5 w-[22rem] max-w-[92vw]"
            ...
            >
              ...
              <p className="text-caption text-white/45 text-center mb-3">岛屿读着你留下的记录，据实回答</p>
              ...
              {(loading || answer) && (
                <div className="rounded-card panel-glass-1 px-4 py-3 text-[14px] leading-relaxed text-white/85 min-h-[3.2rem]">
                  {loading ? <span className="text-white/45">岛屿在翻看你的记录…</span> : answer}
                </div>
              )}

              <button onClick={() => setOpen(false)} className="btn-link w-full text-center text-white/45 text-caption mt-3">
                收起
              </button>
            </motion.div>
          </motion.div>
