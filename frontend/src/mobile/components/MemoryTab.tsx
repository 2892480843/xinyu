...
function Row({ title, desc, onClick }: { title: string; desc: string; onClick: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="panel-glass-1 flex min-h-[60px] w-full items-center justify-between rounded-card px-4 py-3 text-left transition active:scale-[0.98]"
    >
...
export default function MemoryTab({ memoryCount, onMindMap, onTimeMachine, onIslandMap, onPhrases, onLetter, assistant }: Props) {
  return (
    <div className="flex flex-1 flex-col pt-2">
      ...
      <div className="my-auto flex flex-col gap-3 py-4">
        ...
        {memoryCount > 0 && assistant && (
          <div className="panel-glass-1 flex min-h-[60px] w-full items-center justify-between rounded-card px-4 py-3">
            <span className="min-w-0">
