...
- Batch 3：**陪伴精灵**跟随玩家(`Companion` 组件，`useFrame` lerp 到玩家身边 + 浮动 + 发光 + 保留半透材质)；**Q版主角 glb 已接成玩家角色 + 会动**(`GltfAvatar`：按材质槽 `Skin/Hair/Shirt/Pants` 套 toon + 随「捏人」实时改色；只换 Player，NPC/捏人预览仍程序化；**注意 glb 脸朝 -Z**(Blender +Y→glTF -Z)，已转 180° 对齐成 +Z 朝移动方向)。**主角重导出成「四肢独立节点」(LegL/LegR/ArmL/ArmR,枢轴在髋/肩)**：走路时 Player 在 useFrame 里摆腿 + 反相摆臂;**跳跃**(空格/右下 ⤴ 按钮 → 起跳初速 + 重力抛物 + 收腿抬臂姿态;空格原本是送心愿,已改成 E 专用)。

**家园岛 3D 皮（`Island3D.tsx`，2D 底图之上的真 3D 岛屿皮肤）**：
- 整景岛屿 glb `xy_scene_island.glb` 实时 toon 化（`toonifyIsland` 按材质名套 `MeshToonMaterial` + 自发光保辉，雪峰入镜、水下辉光垂入水线倒影；`?island=proc` 可切回程序化地形兜底）。
- 背景**生灵与天体**：漂浮云朵 `xy_bg_cloud` + 飞鸟 `xy_bg_bird` + 海龟 `xy_creature_turtle` + 暖阳 `xy_bg_sun`，各自 `useFrame` 漂浮/巡游动画，统一经 `toonClone` cel-shade。
- 构建脚本：`blender/xy_island_home.py` → `xy_scene_island.glb`；`blender/xy_background.py` → 海龟/飞鸟/云朵/暖阳。

**未接（素材已就绪，可按需推进）**：Batch 2 其余仪式物(纸船/羽毛/叶笺/烛火)、Batch 2 自然 Kit、Batch 3 草帽、Batch 4 小镇道具。

_本清单初版基于 `feat/island-imprint-gameplay` 分支扫描生成。Batch 1（13）+ 2（21）+ 3（3）+ 4（10）+ 家园岛整景与背景生灵/天体（5）已完成，共 52 个 glb 于 `frontend/public/models/`；Blender 端构建脚本见 `blender/`。_
