...
触发条件（满足其一即触发）：
1. 高风险情绪（全部 6 类负面情绪 sad/angry/anxious/helpless/tired/lonely）且强度达到阈值；
2. 命中高风险关键词黑名单（自伤/绝望/无望/被动意念/伤人类表达）——不依赖情绪与强度，
   修复「平静措辞、或被模型误分类为低风险情绪的危机表达漏过」的安全漏洞。

注意：调用方务必传入「服务端情绪分析」得到的 emotion/intensity，绝不可用客户端传入的
emotion 做安全门（否则可用 emotion='happy' 关掉阈值安全网）。
"""
